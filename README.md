...
```
[Claude Code] ──MCP──> [MCP サーバー (TypeScript)] ──> [PostgreSQL + pgvector]
                              ↑
                              └── ETL バッチ ── @pkmn/data
...
- **DB**: ローカル PostgreSQL + pgvector (Docker compose)。pgvector は将来の「役割類似度検索」「類似構築検索」用
- **ETL**: 公式・Showdown・Smogon の更新を定期取り込み
...
- **M1** リポジトリ初期化、Docker compose (Postgres+pgvector)、DB スキーマ、ETL スケルトン
...
- ランタイム: Node.js
- DB: **PostgreSQL + pgvector** (Docker compose)
